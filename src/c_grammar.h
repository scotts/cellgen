...
struct c_grammar: public grammar<c_grammar> {
    ...
    struct definition {
        ...
        //            )
        //        ;
            postfix_expression = root_node_d[
        ...
        // parser start symbol
            translation_unit
                = root_node_d[ *external_declaration ];
        }
	...
	rule<ScannerT, parser_tag<ids::array_index> > array_index;
	rule<ScannerT, parser_tag<ids::for_loop> > for_loop;
	rule<ScannerT, parser_tag<ids::declaration> > declaration;
	rule<ScannerT, parser_tag<ids::identifier> > IDENTIFIER;
	rule<ScannerT, parser_tag<ids::compound> > compound_statement;
	rule<ScannerT, parser_tag<ids::postfix_expression> > postfix_expression;
	rule<ScannerT, parser_tag<ids::expression_statement> > expression_statement;
	rule<ScannerT, parser_tag<ids::statement> > statement;
	rule<ScannerT, parser_tag<ids::assignment_expression> > assignment_expression;
...
struct c_compound_grammar: grammar<c_compound_grammar> {
	c_grammar _c_code;
	...
	struct definition {
		...
		definition(c_compound_grammar const& self): 
			c_code(self._c_code) 
			{}

		const rule<ScannerT, parser_tag<ids::compound> >& start() const
		{
			return c_code.compound_statement;
		}
	};
};
